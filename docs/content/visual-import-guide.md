...
- 内容图片统一使用 `max-width: 100%; height: auto`，不要单独指定固定显示高度。
- 工作流封面与项目图库的 `width`、`height` 填写图片文件的真实像素尺寸；替换图片时同步更新。
- 固定尺寸的作品卡片使用 `object-fit: cover` 等比裁切；灯箱使用 `object-fit: contain` 完整显示。不要改成 `fill`。
- 每次新增或替换素材后执行 `npx playwright test tests/e2e/image-ratio.spec.ts`。测试从首页和档案页发现工作流、项目链接，检查详情封面和项目图库在 390、768、1440px 宽度下的实际比例与图片加载状态。
- 新增独立页面或图片组件时，应将其加入测试覆盖，不能只依赖已有页面通过。
